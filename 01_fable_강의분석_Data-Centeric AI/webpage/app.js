/* ===== Data-Centric AI 세미나 해설 · 앱 로직 =====
   - 데이터는 data.js 의 window.SEMINAR_DATA 로 주입됨 (fetch 미사용, file:// 동작)
   - 뷰: 슬라이드 / 📚개념사전 / ⭐강조 / 🧭연구흐름 / 검색 / Q&A / 참고문헌 / 내 메모
*/
(function () {
  "use strict";
  ...
  function renderText(raw) {
    var html = esc(raw);
    // (배경지식)·(모델 지식) 마커 → 회색 배지 (슬라이드 근거와 구분)
    html = html.replace(/\(배경지식\)/g, '<span class="bg-badge">배경지식</span>');
    html = html.replace(/\(모델 지식\)/g, '<span class="bg-badge model-know">모델 지식</span>');
  ...
  // 슬라이드에 매핑된 개념 → "개념 더 보기" 칩(개념 사전으로 점프)
  function conceptChipsHTML(ids) {
    var valid = (ids || []).filter(function (cid) { return conceptById[cid]; });
    if (!valid.length) return "";
    var chips = valid.map(function (cid) {
      var c = conceptById[cid];
      return '<a class="concept-chip" href="#concept-' + esc(cid) + '" data-concept="' + esc(cid) +
             '">' + esc(c.name) + '</a>';
    }).join(" ");
    return '<div class="ep-h">개념 더 보기</div><div class="concept-chips">' + chips + '</div>';
  }
  ...
  function renderExplain(s) {
    ...
    // 개념 사전 연결(핵심 용어 아래) — 해설 옆 "개념 더 보기"
  ...
  document.addEventListener("click", function (e) {
    ...
    if (cc) { e.preventDefault(); location.hash = "#concept-" + cc.dataset.concept; return; }
  ...
  function onHash() {
    ...
    if (!raw) return;
    var cm = raw.match(/^concept-(.+)$/);
    if (cm && conceptById[cm[1]]) {
      showView("concepts");
      selectConcept(cm[1], true);
      return;
    }
    if (D.slides[raw]) {
      showView("slides");
  ...
  function conceptCategories() {
    ...
    (D.concepts || []).forEach(function (c) {
      ...
      if (!set[cat]) { set[cat] = []; seen.push(cat); }
      set[cat].push(c);
    });
    return { order: seen, map: set };
  }
  function buildConceptList() {
    var box = document.getElementById("conceptList");
    var grouped = conceptCategories();
    var html = '<div class="cl-count">' + (D.concepts || []).length + '개 개념</div>';
    grouped.order.forEach(function (cat) {
      html += '<div class="cl-cat">' + esc(cat) + '</div>';
      grouped.map[cat].forEach(function (c) {
        html += '<button class="cl-item" data-cid="' + esc(c.id) + '">' + esc(c.name) + '</button>';
      });
    });
    box.innerHTML = html;
    box.onclick = function (e) {
      var b = e.target.closest(".cl-item");
      if (b) location.hash = "#concept-" + b.dataset.cid;
    };
  }
  function selectConcept(id, scroll) {
    var c = conceptById[id];
    if (!c) return;
    conceptSel = id;
    // 목록 활성화
    Array.prototype.forEach.call(document.querySelectorAll(".cl-item"), function (b) {
      b.classList.toggle("on", b.dataset.cid === id);
      if (b.dataset.cid === id && scroll) b.scrollIntoView({ block: "nearest" });
    });
  ...
  function renderConceptDetail(c) {
    ...
    var h = '';
    if (c.category) h += '<span class="cd-cat">' + esc(c.category) + '</span>';
    if (c.web_verified) h += '<span class="cd-verified" title="웹 검색으로 출처를 확인한 개념입니다">웹 검증</span>';
    h += '<h2 class="cd-name">' + esc(c.name) + '</h2>';
    if (c.aliases && c.aliases.length) {
      h += '<div class="cd-aliases">별칭: ' + c.aliases.map(function (a) {
        return '<span class="cd-alias">' + esc(a) + '</span>';
      }).join(" ") + '</div>';
    }
    if (c.easy_def) h += '<div class="cd-easy">' + renderText(c.easy_def) + '</div>';
    if (c.detail) h += '<div class="cd-h">자세히</div><div class="cd-body">' + renderText(c.detail) + '</div>';
    if (c.in_this_seminar) {
      h += '<div class="cd-h">이 세미나에서는</div><div class="cd-body cd-seminar">' +
           renderText(c.in_this_seminar) + '</div>';
    }
    if (c.misconceptions) {
      h += '<div class="cd-h">흔한 오해</div><div class="cd-body cd-misc">' +
           renderText(c.misconceptions) + '</div>';
    }
    if (c.sources && c.sources.length) {
      h += '<div class="cd-h">출처</div><div class="cd-sources">';
      c.sources.forEach(function (s) {
        var why = s.why ? ' <span class="cd-why">— ' + esc(s.why) + '</span>' : '';
        if (s.url) {
          h += '<div class="cd-src"><a href="' + esc(s.url) + '" target="_blank" rel="noopener">' +
               esc(s.title || s.url) + '</a>' + why + '</div>';
        } else {
          h += '<div class="cd-src"><span>' + esc(s.title || "(출처)") + '</span>' + why + '</div>';
        }
      });
      h += '</div>';
    }
    if (c.related_concepts && c.related_concepts.length) {
      var rel = c.related_concepts.filter(function (rid) { return conceptById[rid]; });
      if (rel.length) {
        h += '<div class="cd-h">관련 개념</div><div class="concept-chips">';
        h += rel.map(function (rid) {
          return '<a class="concept-chip" href="#concept-' + esc(rid) + '" data-concept="' + esc(rid) +
                 '">' + esc(conceptById[rid].name) + '</a>';
        }).join(" ");
        h += '</div>';
      }
    }
  ...
  function buildConcepts() {
    buildConceptList();
    // 초기 상세: 아직 선택 없으면 첫 개념
    if (!conceptSel && (D.concepts || []).length) {
      selectConcept(D.concepts[0].id, false);
    } else if (conceptSel) {
      selectConcept(conceptSel, false);
    }
  }
  ...
  // 슬라이드에 매핑된 개념 사전 텍스트(검색 가중용)
  function conceptTextOf(s) {
    return (s.concept_ids || []).map(function (cid) {
      var c = conceptById[cid];
      return c ? (c.name + " " + (c.aliases || []).join(" ") + " " + (c.easy_def || "")) : "";
    }).join(" ").toLowerCase();
  }
  ...
  function buildRefs() {
    ...
    var groups = {};
    var labelMap = { paper: "논문", "blog/news": "블로그 / 뉴스", tool: "도구 / 코드", docs: "문서 / 자료", etc: "기타" };
    var orderTypes = ["paper", "blog/news", "tool", "docs", "etc"];
    ...
    typeKeys.forEach(function (t) {
      ...
      groups[t].forEach(function (r) {
        ...
        var verified = r.verified_manual
          ? ' <span class="ref-verified" title="원문 URL을 수동 검증한 참고문헌입니다">검증 등재</span>' : '';
        html += '<div class="ref-row">' +
          '<div class="r-label">' + esc(r.label || r.id) +
          (r.title ? ' <span class="r-title">' + esc(r.title) + '</span>' : '') + verified + '</div>' +
  ...
  function buildHighlights() {
    ...
    // 정의된 순서 + 정의 밖 카테고리는 등장 순서로 뒤에 붙임
    ...
    cats.forEach(function (c) {
      ...
      arr.forEach(function (it) {
        ...
        html += '<div class="hl-card ' + (HL_CAT_CLASS[c] || "") + (it.importance === "high" ? " is-high" : "") + '">' +
          ...
          (it.importance === "high" ? '<span class="hl-star" title="특히 강조한 지점">★</span>' : '') +
  ...
  function exportMemos() {
    ...
    a.href = url;
    a.download = "내메모_DataCentricAI세미나.md";
  ...
  function init() {
    ...
    document.getElementById("footerMeta").textContent =
      D.meta.title + " · 총 " + D.meta.total_slides + "장 · 개념 " +
      (D.concepts || []).length + "항목 · 정적 SPA";
    loadLayout();
    wireSplitters();
    buildTOC();
    buildConcepts();
    ...
    wireImport();
    // 초기 뷰: 해시가 슬라이드/개념이면 그곳으로, 아니면 첫 슬라이드
    var raw = (location.hash || "").replace(/^#/, "");
    var cm = raw.match(/^concept-(.+)$/);
    if (cm && conceptById[cm[1]]) {
